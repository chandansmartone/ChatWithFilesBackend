...
const {
  ...
  parseXLSX,
  parseTXT
} = require('../utils/fileParser'); // Add parseJS function
...
router.post('/upload', authenticateJWT, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (!req.file.buffer) {
      return res.status(400).send('File buffer is undefined.');
    }
    ...
    blobStream.on('finish', async () => {
      ...
      let extractedText;
      try {
        const fileBuffer = Buffer.from(req.file.buffer); // Ensure it's an instance of Buffer
        ...
        if (req.file.mimetype === 'application/pdf') {
          ...
          extractedText = await parseJS(fileBuffer);
        } else if (req.file.mimetype === 'text/css') {
          extractedText = await parseCSS(fileBuffer);
        } else if (req.file.mimetype === 'text/plain') {
          extractedText = await parseTXT(fileBuffer);
